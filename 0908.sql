...
--고객테이블에서 고객아이디에 a문자가 포함된 고객의 고객이름, 등급, 직업을 검색하시오
select 고객아이디, 고객이름, 등급, 직업 from 고객 where 고객아이디 like '%a%';

-- 고객테이블에서 고객이름이 김씨이고 고객아이디에 a문자가 포함되어 있고 등급에i문자가 포함된 고객의 고객이름, 등급, 직업을 검색하시오
...
--고객테이블에서 직업명이 '사'자로 2글자인 고객의 고객이름, 직업, 등급을 검색하시오
...
--***입력뵌값이 없는 컴럼(필드)은 비교연산 결과가 false라서 검색이 되지 않는다
...
--***order by절 (정렬)
--asc(오름차순: 작은값 => 큰값), desc(내림차순: 큰 값 => 작은 값)
--null: 오름차순 맨뒤, 내림차순 맨처음

----고객테이블에서 고객이름, 나이를 검색하시오 단 나이를 기준으로 오름차순 정렬
select 고객이름, 나이 from 고객 order by 나이 asc;

----고객테이블에서 고객이름, 나이를 검색하시오 단 나이를 기준으로 내림차순 정렬
select 고객이름, 나이 from 고객 order by 나이 desc;

--고객테이블에서 등급이 gold 또는 vip인 고객의 고객이름, 나이, 등급, 직업을 검색하시오 단, 고객의 등급을 기준으로 오름차순 정렬
...
--주문테이블에서 p03 또는 p06의 제품을 주문하고 배송지가 경기도인 해을 검색하시오 단 주문수량을 기준으로 내림차순 정렬
select * from 주문 where 주문제품 in ('p03','p06') and 배송지 like '%경기도%' order by 수량 desc;

--주문테이블에서 수량이 10개 이상인 주문의 주문고객, 주문제품, 수량, 주문일자를 검색하시오
--단, 주문제품을 기준으로 오름차순 정렬하고 동일 주문제품은 수량을 기준으로 내림차순 정렬
select 주문고객, 주문제품, 수량, 주문일자 from 주문 where 수량 >=10 order by 주문제품 asc, 수량 desc;

--주문체이블에서 a 또는 b로 시작하고 수량이 10개이상인 주문의 주문고객, 주문제품, 수량, 배송지를 검색하시오
--단,주문고객을 기준으로 내림차순정렬하고 동일 주문고객은 주문제품을 기준으로 내림차순 정렬
select 주문고객, 주문제품, 수량, 배송지 from 주문 
where 주문고객 like 'a%' or 주문고객 like 'b%' and 수량 >= 10 
order by 주문고객 desc, 주문제품 desc;

--집계함수
--conunt(),max(),min(),sum(), avg()
--select절,having절에서만 사용가능
--null값이 들어있는 컴럼의 값은 제외하고 계산

--제품테이블에서 단가의 평균을 검색
--소수점 둘쨰자리에서 반올림, round(avg(단가), 2)
select * from 제품;
...
select sum(재고량) "2500원이상인 제품의 단가의 합계"from 제품 where 단가 >=2500;
