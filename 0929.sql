...
--고객테이블에 고객아이디는 grapes, 고객이름 김포도, 등급 vip, 직업은 의사, 적립금 1500원인 고객의 정보를 삽입(추가)하시오
--속성(컬럼, 필드) 리스트를 작성하여 추가
INSERT into 고객(고객아이디, 고객이름, 직업, 나이, 등급, 적립금) values ('grapes', '김포도','의사','vip',1500);
--INSERT into 고객(고객아이디, 고객이름, 직업, 나이, 등급, 적립금,나이) values ('grapes', '김포도','의사', null,'vip',1500);
...
--데이터업데이트(수정)
--제품테이블에서 제품번호가 p05인 재품명을 신나라면, 가격 2000원으로 수정
...
--delete(행삭제)
--한빛제품테이블에서 단가가 1000원이상 2500원이하인 행들 삭제
--delete from 한빛식품 where 단가>=1000 and 단가 <=2500;
...
--제품테이블에서 제조업체별 제품수로 구성된 업체별제품수라는 이름의 View를 생성
create view 업체별제품수(재조업체, 제품수)
...
--제품2 view에 제품번호 p08, 재고량 1000, 제조업체 신선식품인 제품의 정보를 삽입
select * from 제품1;
